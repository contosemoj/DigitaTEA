        ...
        let isProgressiveMode = false; // Novo: Se modo aprender está ativo
        let isSecondPass = false;      // Novo: Se estamos na volta sem dicas
        ...
        function finishSetup() {
            ...
            isProgressiveMode = checkboxProg.checked; // Lê configuração do modo aprender
        ...
        function initLevel(lvl) {
            currentLevel = lvl;
            isSecondPass = false; // Reinicia controle de voltas
        ...
        function prepareGameQueue() {
            ...
            // Lógica do Modo Aprender: Força dicas na primeira volta
            if (isProgressiveMode && !isSecondPass) {
                configHints = true;
            }

            // Só zera as estatísticas se NÃO for a segunda volta, para acumular
            if (!isSecondPass) {
        ...
        function nextGameItem() {
            ...
            if (gameQueue.length === 0) {
                // VERIFICAÇÃO DO MODO APRENDER
                if (isProgressiveMode && !isSecondPass) {
                    isSecondPass = true; // Ativa flag da segunda volta
                    configHints = false; // Desativa dicas
                    showIntermission();  // Mostra tela de transição
                    return;
                }

                // Final normal da fase
                checkEndPhase();
                isSecondPass = false; // Reset de segurança
                return;
            }
        ...
        // --- NOVA FUNÇÃO: Inicia a Segunda Volta ---
        function startSecondPass() {
            // Recarrega palavras
            gameQueue = [...database[currentLevel].items].sort(() => Math.random() - 0.5);

            // Zera estatísticas desta fase para a avaliação final ser justa
            phaseTotalAttempts = 0;
            phaseCorrectAttempts = 0;
            updateStatsUI();

            showScreen('GAME');
            renderKeyboard(); // Renderiza teclado (agora sem dicas)
            nextGameItem();
        }
